...
| **Settings → Recognition Providers** | What identifies a spinning record: autonomous (local-only) mode, chain mode/order/min-confidence, ACRCloud/AudD credentials, up to 3 custom HTTP providers (auth, request, response field mapping, paste-a-sample auto-fill), local-index/provider usage KPIs. | `GET/PATCH /identity/recognition/{providers,settings}`, `PUT …/providers/{id}/credentials`, `PUT …/providers/order`, `POST/PUT/DELETE …/custom-providers(/{slug})`, `GET /identity/observability/providers` |
